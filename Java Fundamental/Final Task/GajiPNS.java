...
public class GajiPNS {
  ...
  public static void main(String[] args) {
    ...
    // System.out.print("Input Status Pernikahan: ");
    ...
    // jumlahAnak = Integer.valueOf(input.nextLine());
  ...
  // public static String kalkulasiGajiPokok(int masaKerja, String tipeGolongan,
  // int golonganPangkat) {
  ...
  // // ambil value gaji pokok dan format gaji pokok
  // double gajiPokokValue = gajiPokok[golonganIndex - 1][tipeGolonganIndex -
  // 1][masaKerjaIndex];

  // return String.format("%.2f", gajiPokokValue);
  // }
  ...
  public static double getFormattedGajiPokok(int masaKerjaIndex, int tipeGolonganIndex, int golonganIndex) {
    ...
    return gajiPokok[golonganIndex - 1][tipeGolonganIndex - 1][masaKerjaIndex];
  }

  // // kalkulasi tunjangan istri
  // public static double kalkulasiTunjanganIstri(String statusPernikahan, int
  // golonganPangkat) {
  // double[] tunjanganIstri = { 0.0, 0.1, 0.1, 0.15, 0.2 };

  // if (isValidGolonganPangkat(golonganPangkat)) {
  // int indexGolonganPangkat = golonganPangkat - 1;
  // int indexStatusPernikahan = convertStatusPernikahanToInt(statusPernikahan);
  // return tunjanganIstri[indexGolonganPangkat] * indexStatusPernikahan;
  // }
  ...
  // double maxTunjanganAnakPercentage =
  // tunjanganAnakPercentage[tunjanganAnakPercentage.length - 1];
  // double tunjanganAnak;

  // if (jumlahAnak <= 2) {
  // tunjanganAnak = gajiPokok * tunjanganAnakPercentage[jumlahAnak - 1];
  // } else {
  // tunjanganAnak = gajiPokok * maxTunjanganAnakPercentage;
  // }
  ...
  public static double kalkulasiTunjanganAnak(int jumlahAnak, double gajiPokok) {
    double[] tunjanganAnakPercentage = { 0.0, 0.02, 0.02 };
    double maxTunjanganAnakPercentage = tunjanganAnakPercentage[tunjanganAnakPercentage.length - 1];
    double tunjanganAnak;
    System.out.println("gajiPokok" + gajiPokok);

    if (jumlahAnak <= 2) {
      tunjanganAnak = tunjanganAnakPercentage[jumlahAnak - 1] * gajiPokok;
    } else {
      tunjanganAnak = maxTunjanganAnakPercentage * gajiPokok;
    }
  ...
  public static double kalkulasiTunjanganBeras(int golonganPangkat) {
    double[] tunjanganBeras = { 0.0, 10.0, 10.0, 15.0, 20.0 };

    if (isValidGolonganPangkat(golonganPangkat)) {
      int indexGolonganPangkat = golonganPangkat - 1;
      return tunjanganBeras[indexGolonganPangkat];
    }
  ...
  public static double kalkulasiTotalGaji(int masaKerja, String tipeGolongan, int golonganPangkat,
      String statusPernikahan, int jumlahAnak) {
    double gajiPokok = kalkulasiGajiPokok(masaKerja, tipeGolongan, golonganPangkat);
    double tunjanganKeluarga = kalkulasiTunjanganKeluarga(statusPernikahan, masaKerja, tipeGolongan, golonganPangkat);
    double tunjanganAnak = kalkulasiTunjanganAnak(jumlahAnak, gajiPokok);

    double total = gajiPokok + tunjanganKeluarga + tunjanganAnak;
  ...
  public static void displayOutput(String nama, int masaKerja, String tipeGolongan, int golonganPangkat,
      int indexStatusPernikahan, int jumlahAnak, String statusPernikahan) {
    System.out.println("===================================");
    ...
    System.out.printf("Tunjangan Anak: %.2f\n", kalkulasiTunjanganAnak(jumlahAnak, jumlahAnak));
    System.out.printf(
        "Total Gaji: %.2f\n",
        kalkulasiTotalGaji(masaKerja, tipeGolongan, golonganPangkat, statusPernikahan, jumlahAnak));
    System.out.println("===================================");
  }
}
